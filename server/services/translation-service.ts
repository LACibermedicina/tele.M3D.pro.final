...
export async function translateBatch(
  ...
  if (targetLang === 'pt' || !texts.length) return {};
  ...
  for (const text of texts) {
    const cacheKey = `batch:${pageKey}:${text}:${targetLang}`;
  ...
  const prompt = `You are a professional medical software UI translator. Translate the following UI text strings from Brazilian Portuguese to ${langName}.

STRICT RULES:
1. Return a JSON object with the SAME keys (t0, t1, t2...) mapping to the translated strings
2. Do NOT translate these proper names/acronyms (keep them exactly as-is): ${PROPER_NAMES.slice(0, 40).join(', ')}
3. Preserve all emoji characters exactly as-is
4. Medical terminology should use the official standard terminology in ${langName}
5. Keep translations concise — these are UI labels, buttons, headings, and short descriptions
6. Return ONLY valid JSON — no explanations, no code blocks, no markdown wrapping
    ...
    if (result && typeof result === 'object') {
      uncached.forEach((text, i) => {
        ...
        if (typeof translated === 'string' && translated.length > 0) {
          ...
          translationCache.set(`batch:${pageKey}:${text}:${targetLang}`, translated);
        }
      });
    }
  } catch (error) {
    console.error(`Batch translation error for ${pageKey}:${targetLang}:`, error);
    try {
      const textResult = await geminiService.generateText(prompt);
      let cleaned = textResult.trim();
      if (cleaned.startsWith('```json')) cleaned = cleaned.slice(7);
      ...
      console.error(`Batch translation fallback failed for ${pageKey}:${targetLang}:`, fallbackError);
    }
  }
