...
- **Interactive Dashboard System:** Draggable dashboard panels with `DraggableDashboardPanel` wrapper across all dashboards (main, clinical, FHIR, admin, pharmacy, doctor-office, mobile/desktop role dashboards). `MinimizedPanelsContext` manages panel minimization with `MinimizedPanelDock` lateral icon strip. `UnifiedToolbox` floating navigation bar with magnetic edge docking (10% viewport threshold), role-filtered nav groups, collapse mode, and **detachable items** — individual nav items can be dragged out of the toolbox as standalone floating mini-panels with their own drag/minimize/close controls (close returns item to toolbox). Floating widgets (Chatbot, Study Notes, ECG Analyzer, Radiology Analyzer) support minimize-to-dock. Storage keys: `draggable_dashboard_{key}_{id}`, `minimized_panels`, `minimized_dock_side`, `unified_toolbox`, `unified_toolbox_dock_mode`, `unified_toolbox_visible`, `unified_toolbox_detached`, `detached_nav_{path}`. Components: `client/src/components/dashboard/draggable-dashboard-panel.tsx`, `client/src/components/layout/unified-toolbox.tsx`, `client/src/components/layout/minimized-panel-dock.tsx`, `client/src/contexts/MinimizedPanelsContext.tsx`, `client/src/hooks/use-draggable.ts`.
