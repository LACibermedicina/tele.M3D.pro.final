...
- **IAM3D Voice Assistant:** Full-screen voice-driven AI assistant with animated sphere, leveraging Web Speech API. Unified with chatbot capabilities: uses `/api/chatbot/message` for authenticated users and `/api/chatbot/visitor-message` for visitors. Supports scheduling confirmation, urgent consultation requests with on-duty doctors (via `/api/chatbot/urgent-consultation`), profile navigation, and action buttons. Role-based capability badges (Triagem/Agendar/Urgente for patients, Diagnóstico/Protocolos/Plantão for doctors). Conversation persistence via conversationId.
