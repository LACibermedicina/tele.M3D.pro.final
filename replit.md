...
- **FHIR R4 Dashboard + ECG Analysis Engine:** Medical dashboard at `/fhir-dashboard` (doctor/admin only) with 3-column responsive layout. Left sidebar: navigation tabs (Patients, Exams, ECG Engine, Export). Center: FHIR R4 patient table with CRUD via proxy to external FHIR server (`https://r4.ontoserver.csiro.au/fhir`), search, create/delete patients and browse observations. Right: ECG metrics cards, diagnosis probability pie chart, technical/simple summaries. ECG Engine: drag-and-drop dropzone for ECG images, sends base64 to `POST /api/ecg/analyze` which uses GPT-4o vision with 10-step analysis prompt returning structured JSON (metrics, diagnosis probabilities with %, annotation instructions, summaries in Portuguese médico). Recharts visualizations for diagnosis probabilities (bar + pie charts). FHIR Bundle JSON export (auto-download) including ECG observations with LOINC code 100974-8. Fixed medical disclaimer. API: `GET/POST/DELETE /api/fhir/patients`, `GET/POST /api/fhir/observations`, `GET /api/fhir/export-bundle/:patientId`, `POST /api/ecg/analyze`. Express body parser limit increased to 20mb for base64 image uploads. Page: `client/src/pages/fhir-dashboard.tsx`. OpenAI method: `analyzeECGImage()` in `server/services/openai.ts`.
