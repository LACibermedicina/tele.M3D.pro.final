...
- **Desktop OS-Style Environment:** macOS-inspired immersive background (dark indigo/slate gradient with animated ambient orbs and tele.m3d.pro watermark logo). All desktop panels and pages use semi-transparent glass morphism (`backdrop-blur`, `hsla` backgrounds) via CSS utility classes (`desktop-glass-panel`, `desktop-glass-page`, `desktop-glass-footer`). Glass CSS variables defined in `:root` and `.dark` for theme compatibility. `DesktopBackground` component (`client/src/components/layout/desktop-background.tsx`) renders behind all content at `z-0`, desktop-only.
