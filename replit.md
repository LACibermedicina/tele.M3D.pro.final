...
- **Temporary Visitor Access:** Visitors cannot see the Sala de Espera unless logged in or possessing a temporary access link. Doctors/admins generate 2-hour expiring links via "Gerar Link de Acesso Temporário" button on the dashboard. Backend: `POST /api/temporary-access/generate`, `GET /api/temporary-access/validate/:code`. Access code stored in `sessionStorage` with timestamp; auto-expires after 2 hours.
