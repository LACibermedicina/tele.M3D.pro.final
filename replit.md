...
- **FHIR R4 Dashboard + ECG Analysis Engine:** Medical dashboard with FHIR R4 patient/observation CRUD, clinical history timeline, and an ECG analysis engine using AI (Gemini 2.0 Flash/OpenAI gpt-4o-mini) to interpret ECG images with 9-step systematic criteria (ritmo, FC, eixo QRS, onda P, intervalo PR, complexo QRS, segmento ST, onda T, intervalo QT), AHA/ESC/SBC epidemiological data, color-coded annotations (semantic colors: red=ischemia, blue=hypertrophy, green=normal, yellow=moderate risk, purple=arrhythmia), action plans (immediate/follow-up/monitoring), and clinical comments. Includes Recharts visualizations and FHIR Bundle JSON export. Global floating Quick ECG Analyzer and Study Notes widgets.
- **Radiology Imaging Analysis Module:** AI-powered radiographic analysis (Gemini 2.0 Flash primary, OpenAI gpt-4o-mini fallback) with structured output: radiology findings, anatomical overlay, probabilistic diagnosis, prognostic estimation, formal CBR/RSNA report, lay summary, educational notes, multi-specialty relevance, technical quality scoring, color-coded regions, and action plans. Includes global floating Quick Radiology Analyzer widget (Scan icon), FHIR dashboard "Radiologia" tab, and backend routes for analyze/associate/share. Doctor notes folders: radiology_study, radiology_shares.
