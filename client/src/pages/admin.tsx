                      ...
                      <TableRow key={user.id} data-testid={`user-row-${user.id}`}>
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">
                            {user.username}
                            {user.username === 'root' && (
                              <Badge className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs">Super</Badge>
                            )}
                          </div>
                        </TableCell>
                        <TableCell>
...
function AccessControlTab() {
  ...
  const { data: controls = [], isLoading } = useQuery<AccessControl[]>({
    queryKey: ['/api/admin/access-controls'],
  });
  ...
  const updateControlMutation = useMutation({
    mutationFn: async ({ key, value, roles }: { key: string; value: string; roles?: string[] }) => {
      const body: any = { controlValue: value };
      if (roles !== undefined) body.affectedRoles = roles;
      return apiRequest('PUT', `/api/admin/access-controls/${key}`, body);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/access-controls'] });
      queryClient.invalidateQueries({ queryKey: ['/api/access-controls/check'] });
  ...
  const toggleTestUserMutation = useMutation({
    mutationFn: async ({ id, isTestUser }: { id: string; isTestUser: boolean }) => {
      return apiRequest('PATCH', `/api/admin/users/${id}/test-status`, { isTestUser });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/users'] });
  ...
  const suspendTestUsersMutation = useMutation({
    mutationFn: async () => {
      return apiRequest('POST', '/api/admin/suspend-test-users', {});
    },
    onSuccess: async (res: any) => {
      const data = await res.json();
      queryClient.invalidateQueries({ queryKey: ['/api/admin/users'] });
      toast({ title: `${data.count} usuários de teste suspensos` });
    },
    onError: () => {
  ...
  const bulkActionMutation = useMutation({
    mutationFn: async (body: any) => {
      return apiRequest('POST', '/api/admin/users/bulk-action', body);
    },
    onSuccess: async (res: any) => {
      const data = await res.json();
      queryClient.invalidateQueries({ queryKey: ['/api/admin/users'] });
      toast({ title: `Ação aplicada a ${data.count} usuários` });
    },
    onError: () => {
  ...
  const globalControls = controls.filter(c => c.category === 'registration' || c.controlKey === 'site_access_enabled');
  const pageControls = controls.filter(c => c.category === 'access' && c.controlKey !== 'site_access_enabled');
  const creditControls = controls.filter(c => c.category === 'credits');
  const testUsers = allUsers.filter((u: any) => u.isTestUser);
  const allRoles = ['admin', 'doctor', 'patient', 'visitor', 'researcher', 'pharmacist'];

  const handleToggle = (ctrl: AccessControl, newValue: boolean) => {
    updateControlMutation.mutate({ key: ctrl.controlKey, value: String(newValue) });
  };

  const handleRoleToggle = (ctrl: AccessControl, role: string) => {
    const currentRoles = ctrl.affectedRoles || [];
    const newRoles = currentRoles.includes(role)
      ? currentRoles.filter(r => r !== role)
      : [...currentRoles, role];
    updateControlMutation.mutate({ key: ctrl.controlKey, value: ctrl.controlValue, roles: newRoles });
  };

  const minCreditsCtrl = creditControls.find(c => c.controlKey === 'minimum_credits_required');
  const minCreditsMsg = creditControls.find(c => c.controlKey === 'minimum_credits_alert_message');
    ...
    <TabsContent value="access-control" className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Controle o acesso geral ao sistema</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {globalControls.map(ctrl => (
              <div key={ctrl.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                <div>
                  <p className="font-medium text-sm">{ctrl.description || ctrl.controlKey}</p>
                  <p className="text-xs text-muted-foreground">{ctrl.controlKey}</p>
                </div>
                <Button
                  variant={ctrl.controlValue === 'true' ? 'default' : 'destructive'}
                  size="sm"
                  onClick={() => handleToggle(ctrl, ctrl.controlValue !== 'true')}
                >
                  {ctrl.controlValue === 'true' ? 'Ativo' : 'Desativado'}
                </Button>
              </div>
            ))}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Defina o mínimo de créditos TMC para acessar o sistema</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>Quantidade mínima de créditos</Label>
              <div className="flex gap-2">
                <Input
                  type="number"
                  min="0"
                  defaultValue={minCreditsCtrl?.controlValue || '0'}
                  onBlur={(e) => {
                    if (minCreditsCtrl) {
                      updateControlMutation.mutate({ key: 'minimum_credits_required', value: e.target.value });
                    }
                  }}
                  className="max-w-[200px]"
                />
                <span className="flex items-center text-sm text-muted-foreground">TMC</span>
              </div>
              <p className="text-xs text-muted-foreground">0 = sem restrição de créditos</p>
            </div>
            <div className="space-y-2">
              <Label>Mensagem de alerta</Label>
              <Input
                defaultValue={minCreditsMsg?.controlValue || ''}
                onBlur={(e) => {
                  if (minCreditsMsg) {
                    updateControlMutation.mutate({ key: 'minimum_credits_alert_message', value: e.target.value });
                  }
                }}
                placeholder="Mensagem exibida quando usuário não tem créditos suficientes"
              />
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Controle o acesso a áreas específicas do sistema por perfil de usuário</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Status</TableHead>
                <TableHead>Perfis Afetados</TableHead>
                <TableHead className="text-right">Ações</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {pageControls.map(ctrl => (
                <TableRow key={ctrl.id}>
                  ...
                  <TableCell>
                    <Badge variant={ctrl.controlValue === 'true' ? 'default' : 'destructive'}>
                      {ctrl.controlValue === 'true' ? 'Ativo' : 'Bloqueado'}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {allRoles.map(role => (
                        <Badge
                          key={role}
                          variant={(ctrl.affectedRoles || []).includes(role) ? 'default' : 'outline'}
                          className="cursor-pointer text-xs"
                          onClick={() => handleRoleToggle(ctrl, role)}
                        >
                          {role}
                        </Badge>
                      ))}
                    </div>
                    {(!ctrl.affectedRoles || ctrl.affectedRoles.length === 0) && (
                      <p className="text-xs text-muted-foreground mt-1">Sem seleção = afeta todos</p>
                    )}
                  </TableCell>
                  <TableCell className="text-right">
                    <Button
                      variant={ctrl.controlValue === 'true' ? 'outline' : 'default'}
                      size="sm"
                      onClick={() => handleToggle(ctrl, ctrl.controlValue !== 'true')}
                    >
                      {ctrl.controlValue === 'true' ? 'Desativar' : 'Ativar'}
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Gerencie e suspenda usuários marcados como teste</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center">
              <p className="text-sm text-muted-foreground">
                {testUsers.length} usuário(s) de teste
              </p>
              <Button
                variant="destructive"
                size="sm"
                onClick={() => suspendTestUsersMutation.mutate()}
                disabled={suspendTestUsersMutation.isPending || testUsers.length === 0}
              >
                {suspendTestUsersMutation.isPending ? 'Suspendendo...' : 'Suspender Todos Testes'}
              </Button>
            </div>
            {testUsers.length > 0 ? (
                ...
                {testUsers.map((user: any) => (
                  <div key={user.id} className="flex items-center justify-between p-2 rounded border">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => toggleTestUserMutation.mutate({ id: user.id, isTestUser: false })}
                    >
                      Remover Teste
                    </Button>
                  </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Bloqueie ou desbloqueie grupos inteiros de usuários</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={bulkAction} onValueChange={setBulkAction}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="unblock">Desbloquear Todos</SelectItem>
                  <SelectItem value="mark_test">Marcar como Teste</SelectItem>
                  <SelectItem value="unmark_test">Desmarcar Teste</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <Button
              className="w-full"
              disabled={!bulkRole || !bulkAction || bulkActionMutation.isPending}
              onClick={() => {
                if (bulkRole && bulkAction) {
                  bulkActionMutation.mutate({ action: bulkAction, role: bulkRole });
                }
              }}
            >
              {bulkActionMutation.isPending ? 'Aplicando...' : 'Aplicar Ação'}
            </Button>
            <p className="text-xs text-muted-foreground">
              O usuário root nunca será afetado por ações em lote.
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Visão geral dos usuários com indicadores de status</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {allUsers.map((user: any) => (
                  <TableRow key={user.id} className={user.username === 'root' ? 'bg-amber-50 dark:bg-amber-950/10' : ''}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                    ...
                    <TableCell>
                      <span className="text-sm">{user.tmcCredits || 0} TMC</span>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {user.isBlocked && <Badge variant="destructive" className="text-xs">Bloqueado</Badge>}
                        {user.isTestUser && <Badge className="bg-orange-500 text-xs">Teste</Badge>}
                        {!user.isBlocked && !user.isTestUser && <Badge variant="secondary" className="text-xs">Ativo</Badge>}
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      {user.username !== 'root' && (
                        <div className="flex gap-1 justify-end">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => toggleTestUserMutation.mutate({ id: user.id, isTestUser: !user.isTestUser })}
                          >
                            {user.isTestUser ? 'Desmarcar Teste' : 'Marcar Teste'}
                          </Button>
                        </div>
                      )}
                      {user.username === 'root' && (
